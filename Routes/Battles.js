...
router.post("/battle/create", Auth, async (req, res) => {
  const { Game_type, Game_Ammount, id, battleId } = req.body;
    ...
    if (Game_Ammount >= 5 && Game_Ammount <= 10) {
      if (Game_Ammount % 5 === 0) {
        if (user.Wallet_balance >= req.body.Game_Ammount) {
          ...
          //game battle limit...
          let newGame = await Battle.find({
            $or: [
              {
                $and: [
                  { Battle_id: id },
                  { Status: "requested" },
                  { waiting_room: req.user.id },
                ],
              },
            ],
          });
          ...
          if (newGame.length == 0) {
            ...
            const game1 = await Battle.findByIdAndUpdate(
              { _id: id },
              { waiting_room: req.user.id },
              { new: true }
            );
      ...
      res.send({
        msg: "Gamemmm amount should be Greater then 50 and less then 10000",
      });
    }
...
router.get("/challangeBattle/all", Auth, async (req, res) => {
  try {
...
router.get("/battle_challange/:id", Auth, async (req, res) => {
  try {
    let admin = await Battle.find({ Battle_id: req.params.id })
...
router.put("/fee/deduction/:id", Auth, async (req, res) => {
  try {
    // if(InProcessSubmit==false)
    console.log(req.body.Battle_id);
    let battleId = req.body.Battle_id;
    ...
    // add wallet end
    const battle = await Battle.findOneAndUpdate({ Battle_id: battleId },
      {
        Winner_closingbalance : game.Game_Ammount,
        Loser_closingbalance : game.Game_Ammount
      });
...
router.put("/battle/accept/:id/:battleId", Auth, async (req, res) => {
  //  console.log('param Id',req.params.id);
  try {
    const game = await Game.findById(req.params.id);
    const battle = await Battle.updateOne(
      { $and: [{ Battle_id: req.params.battleId }, { Status: "requested" }] },
      {
        Accepetd_By: req.user.id,
    ...
    if (
      (game.Status != "cancelled" &&
        game.Status != "completed" &&
        game.Status != "running" &&
        game.waiting_room != req.user.id &&
        game.Status == "new") ||
      game.Status == "waiting"
    ) {
      const creatorEnrolled = await Battle.find({
        $and: [
      ...
      if (user.Wallet_balance >= game.Game_Ammount) {
        ...
        //   });
        // let newGame = await Battle.find({
        //   $or: [{ $and: [{ Battle_id: id } ,{ Status: 'requested' }, { waiting_room: req.user.id}] }],
        // });
        ...
        // if (battle.length != 0) {
        // battle.Accepetd_By = req.user.id;
        // battle.Acceptor_by_Creator_at = Date.now();
        // battle.Status = 'completed';
        // battle.waiting_room = req.user.id;
        //   // console.log(game);
        //   battle.save();
        //   res.send(battle);
        // }
...
router.put("/battlechallange/running/update/:id", Auth, async (req, res) => {
  try {
    const game = await Battle.findByIdAndUpdate(req.params.id, req.body, {
      new: true,
    });
